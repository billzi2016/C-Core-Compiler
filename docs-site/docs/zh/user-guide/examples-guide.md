...
所以看 `examples/` 时，别把它们当成“跑一下看看有输出没”的 demo，更像一点的话，它们其实是这台编译器拿来给你拆着看的样本盒：
...
这不是重复劳动，也不是作者手痒多写了一份，而是故意把“给机器验证”和“给人看结果”分开了。前者适合做判断，后者适合让你立刻知道自己看到的是个啥。
...
从编译原理角度看，这类示例更像是拿来验证“程序最后算出来的东西对不对”，它不负责把过程演得多热闹。
...
第一次摸这个项目的人，通常会先和这类示例相处得更好，因为它不需要你盯退出码猜结果，屏幕上会直接把东西吐出来。
...
它的重要性在于，它通常已经不再是最基础的 `int` 小练习了，所以很适合拿来感受“为什么一旦语言特性复杂起来，编译路径就开始分叉”。
...
如果你还没对“为什么不是所有东西都走 IR”这件事建立直觉，这个示例很容易让你一下子看出差别，因为它天然就更像那种不适合硬塞进简化 IR 的东西。
...
它在当前项目里很关键，因为它一脚就踩到了更真实的内存模型问题。到了这里，事情明显不再像 `a + b` 那么干净。
...
这样你做的就不只是“把很多示例跑了一遍”，而是在看同一台编译器碰到不同语言结构时，反应会怎么变，哪里还能走主路，哪里已经得拐弯。

## 一句话总结

这个项目里的 `examples/` 真不是一堆零散样例，它更像一组按层次摆好的观察样本。你顺着它看，比较容易看清这台编译器在哪些地方还很朴素，在哪些地方已经开始长出自己的性格。
